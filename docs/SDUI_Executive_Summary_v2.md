...
| **Atomic rendering layer**  | Dual-layer architecture: 12 atomic element types (Container, Text, Image, Button, Spacer, Divider, ScrollContainer, Conditional, DisplayGrid, OverlayContainer, SectionSlot, LiveClock) composed by the server and rendered by `AtomicRouter` on Android, iOS, and Web. `AtomicComposite` section type bridges the section and atomic layers. `SectionSlot` allows atomic trees to embed full section renderers. Stateless layout surfaces (headers, rails, hero panels, promo banners, stat lines, video carousels, schedule layouts, error states, live-score cards) are server-composed as `AtomicComposite` trees driven by `bindRef` leaf-level data resolution and the `LiveClock` primitive for client-owned tick animation where needed | **Done**               |
...
- **Schema version negotiation** — `schemaVersion` in the request envelope; server includes `schemaVersion` in response metadata
...
- **Platform-aware composition** — the request envelope's `platform[name]` (with `platform.deviceClass` and `experiments[*]`) drives per-platform field values (e.g., form layout orientation). Shared schema, shared data pipeline, per-platform-family composition tailoring
