...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| **JSON Schema**             | Complete schema defining 8 atomic primitives, 7 semantic section types, 5 action types, 3 refresh policies, field-level data bindings, and screen-level state management                                                                                                                                  | **Done**               |
...
| **Android renderer**        | Full Kotlin/Compose renderer in a reusable `sdui-core` library module. 7 section renderers (ScoreboardHeader, StatLine, ContentCard, ContentRail, TabGroup, PromoBanner, GameCard). Real-time Ably integration, polling with direct CDN support, StateFlow-based state management, generic action handler | **Done**               |
| **Web renderer**            | React/TypeScript renderer with 6 section renderers, refresh policy hooks, action handler, and data binding applier. Polling works; Ably SSE partially integrated                                                                                                                                          | **Done** (partial SSE) |
