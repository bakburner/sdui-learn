...
public class StatsApiClient {
     ...
     */
    public Map<LocalDate, Integer> parseGameCounts(JsonNode schedule) {
     ...
     */
    public JsonNode normalizeCoreApiFeedToScoreboard(JsonNode feed, LocalDate date) {
    ...
    private JsonNode fetchCdnJson(String url) throws IOException {
        return executeRequest(url, h -> {
            h.set("User-Agent", BROWSER_USER_AGENT);
            h.set("Accept", "application/json,text/plain,*/*");
            h.set("Origin", NBA_ORIGIN);
            h.set("Referer", NBA_REFERER);
        });
    }
    ...
    private JsonNode fetchStatsApiJson(String url) throws IOException {
        return executeRequest(url, h -> {
            ...
            h.set("Origin", NBA_ORIGIN);
            h.set("Referer", NBA_REFERER);
            h.set("Ocp-Apim-Subscription-Key", subscriptionKey);
        });
    }
    ...
    private JsonNode fetchCoreApiJson(String url) throws IOException {
        return executeRequest(url, h -> {
            ...
            h.set("Ocp-Apim-Subscription-Key", coreApiSubscriptionKey);
        });
    }
    ...
    private JsonNode executeRequest(String url, java.util.function.Consumer<org.springframework.http.HttpHeaders> headerCustomizer)
        ...
        try {
            ...
            log.warn("API returned status {} for {}", e.getStatusCode().value(), url);
            return null;
