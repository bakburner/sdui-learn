...
- **Platform-native visual realization** — the shared schema carries semantic design-system tokens (`textVariant: "titleMedium"`, `buttonVariant: "primary"`); each client resolves those tokens into its platform's current design language. iOS looks like iOS, Android looks like Android, web looks like web. When Apple or Google ship a new design language — Liquid Glass, Material 3 Expressive — client resolvers update once and every token-driven screen picks up the new look without server changes or composer churn. Cross-platform pixel parity is not the goal; platform fluency is. `TextVariant` (22 cases) and `ButtonVariant` demonstrate the pattern today. ADR-013 generalizes it to container surfaces (`ContainerVariant`, `ImageVariant`, `DividerVariant`) including platform materials and per-OS-version tiering
