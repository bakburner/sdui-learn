...
| **Semantic sections** | CalendarStrip | Platform-native date picker hosting + in-place reconciliation across parameterized refresh | Client owns calendar arithmetic, programmatic scroll-to-selected, locale-aware formatting; reconciles mutable pager/scroll state in place when section is replaced by id. |
| **Semantic sections** | CalendarMonthList | Platform-native month-grid hosting with client-owned scroll/interaction state | Client owns calendar layout, scroll-to-month, locale-aware first-day rotation, and persistent scroll position across recomposition. |
...
| Android renderer (Compose) | **Built** | Section router + 10 semantic section renderers + AtomicRouter with 12 atomic primitives incl. `LiveClock` and `OverlayContainer`. `IconTokenResolver` + bottom navigation shell resolve `sdui:*` icon tokens to Material Symbols. |
| Web renderer (React) | **Built** | React section router + 10 semantic section renderers + AtomicRouter with 12 atomic primitives incl. `LiveClock`, `OverlayContainer`, and live data wrappers. `IconTokenResolver` + Material Symbols font for top navigation bar. |
| iOS renderer (SwiftUI) | **Built** | Swift Package (`ios/`) with SwiftUI section router + 10 semantic section views + AtomicRouter with 12 atomic primitives incl. `LiveClock` and `OverlayContainer`. Server-declared bottom `SduiNavigationShell` resolves `sdui:*` icon tokens to SF Symbols. Real-time via Ably (`AblyChannelManager` actor) + `PollingDriver`. `SectionVisibilityTracker` wired. Demo app (`SduiDemo`, XcodeGen, `make ios-run`) bootstraps `nba://for-you`. |
...
| Date | Summary |
|---|---|
| 2026-05-27 | Doc consistency audit. Dual-layer type model: section count 10 → 11 with `CalendarMonthList` added to the permanent-with-renderer list. Android, Web, iOS renderer-status rows updated 9 → 10 semantic section renderers/views. §9 section-classification inventory: added missing `CalendarMonthList` row (table previously had `CalendarStrip` only despite the dual-layer description listing both). |
