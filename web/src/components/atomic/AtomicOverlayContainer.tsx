...
function overlayStyle(overlay: AtomicOverlay, el?: Record<string, unknown>): React.CSSProperties {
  ...
  // When the overlay element declares fill sizing modes, stretch the
  // absolutely-positioned wrapper so the child's percentage sizing resolves
  // against the full parent bounds (e.g. a gradient scrim spanning the image).
  if (el?.widthMode === 'fill') {
    style.left = start;
    style.right = end;
    style.width = undefined;
    style.transform = undefined;
  }
  if (el?.heightMode === 'fill') {
    style.top = top;
    style.bottom = bottom;
