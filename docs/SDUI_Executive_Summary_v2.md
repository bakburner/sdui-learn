...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| **JSON Schema**             | Complete schema defining 11 atomic element types (9 rendering primitives + SectionSlot bridge + LiveClock), 9 section types (8 permanent + AtomicComposite), 6 action types, 7 action triggers, 3 refresh policies, field-level data bindings, and screen-level state management. 10 former section types (including GamePanel) migrated to server-composed AtomicComposite (no client renderers). | **Done**               |
...
| **Atomic rendering layer**  | Dual-layer architecture: 11 atomic element types (Container, Text, Image, Button, Spacer, Divider, ScrollContainer, Conditional, DisplayGrid, SectionSlot, LiveClock) composed by the server and rendered by `AtomicRouter` on Android, iOS, and Web. `AtomicComposite` section type bridges the section and atomic layers. `SectionSlot` allows atomic trees to embed full section renderers. Server-side `AtomicCompositeBuilder` migrated 10 former section types (ErrorState, SectionHeader, PromoBanner, ContentRail, FollowingRail, HeroPanel, StatLine, VideoCarousel, NbaTvSchedule, GamePanel) to server-composed atomic layouts. Their schema definitions have been pruned — only the 8 permanent section types + `AtomicComposite` remain in the `Section.type` enum | **Done**               |
| **Request context envelope** | Typed request context (`SduiRequestContext`) with bracket-notation GET params and POST fallback. Client-side `RequestEnvelopeBuilder` on Android, iOS, and web. Fields: platform, appVersion, locale, deviceClass, experiments, traceId — all optional with defaults | **Done**               |
...
- **Two-tier primitives with clear decision criteria** — atomic primitives (Container, Text, Image, Button, Spacer, Divider, ScrollContainer, Conditional, DisplayGrid, SectionSlot, LiveClock) and semantic sections (TabGroup, BoxscoreTable, Form, etc.) coexist via the `AtomicComposite` bridge. The boundary is principled: sections that integrate platform SDKs (e.g., SubscribeHero/SubscribeBanner with In-App Purchase, AdSlot with ad SDKs) or manage client-owned interaction state (e.g., BoxscoreTable sort/scroll, TabGroup tab selection) remain native sections. Stateless layout surfaces migrate to server-composed atomic trees — enabling new layouts without app releases
