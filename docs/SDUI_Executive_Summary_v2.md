...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| **JSON Schema**             | Complete schema defining 10 atomic element types (9 rendering primitives + SectionSlot bridge), 10 section types (9 permanent + AtomicComposite), 6 action types, 7 action triggers, 3 refresh policies, field-level data bindings, and screen-level state management. 9 former section types migrated to server-composed AtomicComposite (no client renderers). | **Done**               |
...
| **Android renderer**        | Full Kotlin/Compose renderer in a reusable `sdui-core` library module. 9 permanent section renderers (TabGroup, GamePanel (with server-driven `displayConfig` controlling layout: logo size, card height, score style, background), BoxscoreTable, Form, SeasonLeadersTable, SubscribeBanner, SubscribeHero, AdSlot, VideoPlayer) plus `AtomicComposite` routing to the atomic layer. Real-time Ably integration, polling with direct CDN support, StateFlow-based state management, generic action handler, server-driven image fallback via Coil `SubcomposeAsyncImage`, `IconTokenResolver` for `sdui:*` icon tokens, server-declared bottom navigation shell | **Done**               |
| **Web renderer**            | React/TypeScript renderer with 9 permanent section renderers plus `AtomicComposite` routing, refresh policy hooks, action handler, data binding applier, and server-driven image fallback via `onError` handlers. `IconTokenResolver` + Material Symbols font for semantic icons; server-declared top navigation bar. Polling works; Ably SSE partially integrated                                                                                                                                         | **Done** (partial SSE) |
...
| **Atomic rendering layer**  | Dual-layer architecture: 10 atomic primitives (Container, Text, Image, Button, Spacer, Divider, ScrollContainer, Conditional, DisplayGrid, SectionSlot) composed by the server and rendered by `AtomicRouter` on Android, iOS, and Web. `AtomicComposite` section type bridges the section and atomic layers. `SectionSlot` allows atomic trees to embed full section renderers. Server-side `AtomicCompositeBuilder` migrated 9 former section types (ErrorState, SectionHeader, PromoBanner, ContentRail, FollowingRail, HeroPanel, StatLine, VideoCarousel, NbaTvSchedule) to server-composed atomic layouts. Their schema definitions have been pruned — only the 9 permanent section types + `AtomicComposite` remain in the `Section.type` enum | **Done**               |
| **Request context envelope** | Typed request context (`SduiRequestContext`) with bracket-notation GET params and POST fallback. Client-side `RequestEnvelopeBuilder` on Android and web. Fields: platform, appVersion, locale, deviceClass, experiments, traceId — all optional with defaults | **Done**               |
| **Experiment assignment**   | Client-authoritative experiment model (ADR-006 accepted). Clients send `experiments` map in request envelope; server resolves via `resolveVariant()` for composition branching. Kill switch is client-side. Exposure tracking via fireAndForget actions | **Done**               |
| **Internationalization (i18n)** | Section-level `stringTable` stamped by server per locale. Server pre-translates all initial text. Clients consume `stringTable` from each section for binding-time resolution. Parameterized strings via server-side atomic decomposition (~1% of strings) | **Done**               |
| **iOS renderer**            | SwiftUI renderer in `ios/Sources/SduiCore/` Swift package. 9 permanent section views plus `AtomicComposite` routing, `SectionRouter` / `AtomicRouter`, server-declared `SduiNavigationShell` (bottom bar), `IconTokenResolver` for `sdui:*` tokens, `RequestEnvelopeBuilder`, impression tracker, `SectionErrorBoundary`, polling driver, Ably integration (gated for x86_64 simulators). Bootstrap URI synced with Android / web (`nba://for-you`). XcodeGen-managed `SduiDemo` host with `make ios-run`. | **Done** (partial SSE) |
...
- **Action system** — navigate, fireAndForget, mutate, dismiss, refresh, toast all dispatched generically through a single handler
...
- **Platform-native visual realization** — the shared schema carries semantic design-system tokens on a uniform `variant: string` property on every atomic element (e.g. `variant: "titleMedium"` on a `Text`, `variant: "primary"` on a `Button`), with the per-primitive vocabulary defined by enums `TextVariant`, `ButtonVariant`, `ContainerVariant`, and `ImageVariant`; each client parses `variant` against the enum that matches the element's `type` and resolves it into its platform's current design language. iOS looks like iOS, Android looks like Android, web looks like web. When Apple or Google ship a new design language — Liquid Glass, Material 3 Expressive — client resolvers update once and every token-driven screen picks up the new look without server changes or composer churn. Cross-platform pixel parity is not the goal; platform fluency is. `TextVariant` (22 cases) and `ButtonVariant` demonstrate the pattern today. ADR-013 generalizes it to container and image surfaces (`ContainerVariant`, `ImageVariant`) including platform materials and per-OS-version tiering
...
| **Design-system style tokens (containers)**         | Platform-native container polish (iOS materials / Liquid Glass, Android Material surfaces, web CSS mixins) as named design-system concepts rather than bags of inline properties. Extends the existing typography/button token pattern to hero cards, surface chrome, etc. | Medium — ADR-013 draft |
...
| Deliverable                                                                                        | Owner               |
| -------------------------------------------------------------------------------------------------- | ------------------- |
| Contract tests for all 10 section types (golden SDUI response fixtures + assertions)               | SDUI Engineering Team           |
...
| **Schema evolution complexity**       | Breaking changes to older app versions                | Additive-only change policy. Version negotiation protocol implemented in prototype. Binding paths that reference removed fields degrade gracefully — see §3a of requirements.        | Version header working; governance process needed at M3         |
...
| Style tokens for atomic primitives   | **ADR-013 Proposed** (draft)      |
