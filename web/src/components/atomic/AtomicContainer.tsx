 ...
 */
export function AtomicContainer({ element, state, onAction }: AtomicProps): React.ReactElement {
  ...
  return (
    <div style={style}>
      {element.children?.map((child, i) => (
        <AtomicRouter key={child.id ?? i} element={child} state={state} onAction={onAction} />
      ))}
    </div>
  );
}
