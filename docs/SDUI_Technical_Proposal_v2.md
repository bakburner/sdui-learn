...
The atomic layer bridges the Figma design system and rendered output through a three-layer token architecture documented in [`sdui-design-system.md`](design/sdui-design-system.md):

- **Layer 1 — Inline primitives.** `padding`, `cornerRadius`, `shadow`, `gap`, `opacity`, `border`, `background`, `aspectRatio` on every element. Sizes and spacings use semantic tokens from per-form-factor registries (`schema/spacing-tokens.json`, `schema/corner-radius-tokens.json`).
...
- **Layer 3 — Color tokens.** `token:nba.color.*` / `token:nba.label.*` references resolved to light/dark hex at render time from the multi-tier Kinetic palette registry (`schema/color-tokens.json`).
...
See [`sdui-design-system.md`](design/sdui-design-system.md) for complete token registries, variant definitions, Figma naming conventions, and the accessibility/i18n checklists.
...
- See `docs/design/sdui-design-system.md` §2 (Box-model cascade) for the cascade rules and ownership boundaries at each level (`Screen.contentInsets` → `Section.surface` → `AtomicBox`).
...
7. Style-token strategy per ADR-013 — **shipped and Accepted.** Three-layer design system (inline primitives, per-primitive variant enums with platform-native realization, color-token registry) built on all platforms. Figma export pipeline deferred — registry is ref-app-seeded with a Kinetic-compatible shape. Reference: [`sdui-design-system.md`](design/sdui-design-system.md).
