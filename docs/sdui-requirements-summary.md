...
- Platform and schema version use `platform[name]` and `schemaVersion` in the envelope (query on GET, body on POST). Clients do not send `X-Platform` or `X-Schema-Version` headers.
...
- **All composition routes are dual-mounted GET + POST to the same handler.** This includes `/sdui/refresh/{screenId}`, the parameterized-refresh endpoint that backs Form-driven section refreshes. There are no GET-only or POST-only composition endpoints.
