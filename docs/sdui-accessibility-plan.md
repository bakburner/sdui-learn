...
|---|---|---|---|
| GamePanelRenderer | ✅ tricode for logos (4 instances) | ❌ | ❌ |
| ScoreboardHeaderRenderer | ✅ `"${teamName} logo"` | ❌ | ❌ |
...
|---|---|---|---|
| GamePanel | ✅ tricode for logos | ❌ | ❌ |
| ScoreboardHeader | ✅ teamName for logos | ❌ | ❌ |
...
6. **Add `liveRegion`** to ScoreboardHeader score text (scores update during live games).

**Priority fixes per renderer:**

| Renderer | Fix |
|---|---|
| GamePanelRenderer | Wrap tappable card in `semantics(mergeDescendants = true) { contentDescription = "$away vs $home" }` |
| ScoreboardHeaderRenderer | Add `liveRegion = LiveRegionMode.Polite` to score text |
...
| Renderer | Fix |
|---|---|
| GamePanel | `role="button"` on clickable cards; `aria-label="${away} vs ${home}"` |
| ScoreboardHeader | `aria-live="polite"` on score region |
...
| `liveRegion` overuse causes announcement fatigue | Screen reader users overwhelmed | Server guidelines limit `liveRegion` to ScoreboardHeader scores and ErrorState messages only |
