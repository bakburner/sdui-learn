...
| **Request context envelope** | Typed `SduiRequestContext` with bracket-notation GET params and POST fallback. `RequestEnvelopeBuilder` on all platforms. Query fields: locale, schemaVersion, deviceClass, capabilities, experiments. Headers: X-Analytics-Platform, X-App-Version, X-OS-Version, X-Trace-Id, X-Request-Id | **Done**               |
