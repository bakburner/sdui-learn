...
| **Android renderer**        | Kotlin/Compose `sdui-core` library. 8 permanent section renderers + AtomicComposite routing. Ably real-time, polling, StateFlow state management, action handler, image fallback, icon tokens, server-declared navigation shell | **Done**               |
